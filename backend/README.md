...
| POST | `/v1/observations` | — | Crowd submission (multipart: `gtin`, `device_id`, `quantity`, `unit_kind`, `raw_text`, `ocr_confidence`, optional `photo` as JPEG ≤5MB). `device_id` must be a UUID (`400 {"error":"invalid_device_id"}` otherwise) and is canonicalised to lowercase before it's stored. Computes a confidence score for review priority; every crowd row returns `pending` until an administrator reviews its stored label photo. Rate-limited to 30/hour per `device_id`. |
| POST | `/v1/devices` | — | Upserts the device row: APNs token, `location_id`, categories, watches, notification prefs, and the App Store transaction JWS. Grants Pro (sets `pro_until`) only when the JWS verifies against a Worker-allowed `environment` (see `APPSTORE_ALLOWED_ENVIRONMENTS` below). When its `appAccountToken` equals the posted `device_id`, Pro is granted normally; when it names a *different* valid device id (a reinstall or new-device restore, since Apple's `appAccountToken` is permanent per subscription), Pro is **rebound**: cleared from whichever row currently holds that token and written onto the posting device instead, so exactly one row is ever Pro for a given token. A verification failure leaves the existing entitlement untouched. Device ids are canonicalised to lowercase on the server; the app may send either case. |
...
| `APPSTORE_ALLOWED_ENVIRONMENTS` | comma-separated `Sandbox`/`Production` | Which App Store `environment` claims `/v1/devices` and `/v1/appstore/notifications` will act on; anything else is ignored (no Pro grant / no state change, still `200`). Code default (when unset) is `"Production"`; `wrangler.toml` currently sets `"Sandbox,Production"` for the TestFlight period — flip it to `"Production"` at App Store release. |
