...
The spec (§1, §3) also marks `SavingsForecast`, the 10-screen quiz onboarding and its "$/yr exposure" reveal, and the `com.shrunk.pro.lifetime` non-consumable **Removed** — but as of this writing `Shrunk/Services/SavingsForecast.swift` and `Shrunk/Features/Onboarding/{OnboardingContainerView,OnboardingViewModel}.swift` still exist, and while `com.shrunk.pro.lifetime` is now gone from `Shrunk/Services/StoreKitService.swift` (replaced by `ShrunkProProduct.monthly`/`.yearly`), it still exists in `Shrunk/Resources/Shrunk.storekit` until Task 7 updates it. Phase 5 (`docs/superpowers/plans/2026-08-26-shrunk-v2-phase5-subscription-onboarding-dashboard.md`) is what replaces them with `pro.monthly` / `pro.yearly` and the new onboarding. Grep `StoreKitService.swift` for `pro.lifetime` before assuming Phase 5 has landed, and don't describe these as removed — or write UI docs against the new onboarding — until it has.
