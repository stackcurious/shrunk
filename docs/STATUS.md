...
Validated for build 6: focused store-picker and Kroger DTO tests passed; the complete deterministic non-StoreKit unit suite passed before release versioning; the generic Release simulator build and signed device archive passed. A later broad XCTest rerun hit the machine's known simulator worker-launch stall before executing tests and was cancelled; it produced no app test failure. The signed development build is installed and open on the paired iPhone 11 for the remaining location/search acceptance.

App Store Connect ASO was re-audited against the live U.S. catalog on September 8 and updated in place. The indexed fields are now name `Shrunk: Shrinkflation Scanner`, subtitle `Grocery sizes & unit prices`, and a full 100-byte keyword field without repeats from the visible fields. Promotional text and the 2,911-byte description were rewritten around supported barcodes, documented size evidence, available Kroger-family pricing, unit costs, and on-device nearby-store search; ASC re-read confirmed the exact saved values and retained `READY_FOR_REVIEW`. The old arrow icon visible in ASC's header is stale presentation: build 5 and build 6 contain identical compiled Scanned Delta icons, and the obsolete icon generator was replaced to prevent regression.

The accuracy and organic-search site update is live in Vercel deployment `dpl_3SdkTEpLiet837zvoZEsQEHCLf8f`. It uses the exact shipping icon, accurate size-versus-price claims, aligned website analytics disclosure, updated store-search support, canonical/OG/Twitter/Smart App Banner metadata, a 30-URL sitemap, and a verified HTTP 200 `robots.txt` that points to that sitemap.
